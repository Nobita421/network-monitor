...
import { Cpu, Trash2, ArrowDown, ArrowUp, AlertCircle } from 'lucide-react'
...
export function ProcessList({ processes }: ProcessListProps) {
  ...
  const handleKill = async () => {
    if (!toKill) return
    setKillError(null)
    const result = await killProcess(toKill.pid)
    if (result.ok) {
      setToKill(null) // Close modal only on success
    } else {
      // Keep modal open and show error inline
      setKillError(result.reason ?? 'Could not terminate process')
    }
  }
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between border-b border-white/[0.04] px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">Estimated by socket share</p>
            </div>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.03] text-[9px] uppercase tracking-[0.15em] text-slate-700">
                <th scope="col" className="px-5 py-3 text-left font-semibold">Process</th>
                <th scope="col" className="px-4 py-3 text-left font-semibold">PID</th>
                <th scope="col" className="px-4 py-3 text-right font-semibold">
                  <span className="flex items-center justify-end gap-1">
                    <ArrowDown size={9} className="text-emerald-600" aria-hidden /> Download
                  </span>
                </th>
                <th scope="col" className="px-4 py-3 text-right font-semibold">
                  <span className="flex items-center justify-end gap-1">
                    <ArrowUp size={9} className="text-sky-600" aria-hidden /> Upload
                  </span>
                </th>
                <th scope="col" className="px-4 py-3 text-center font-semibold">Conns</th>
                <th scope="col" className="px-5 py-3 text-center font-semibold">Kill</th>
              </tr>
            </thead>
            <tbody>
              {processes.length === 0 ? (
                ...
                >
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gradient-to-br from-slate-800 to-slate-900 border border-white/[0.05] text-[9px] font-bold text-slate-500 font-data" aria-hidden>
                        {idx + 1}
                      </div>
                      <span className="font-medium text-[13px] text-slate-200 truncate max-w-[120px]">{proc.name}</span>
                      {proc.isEstimated && (
                        <span className="rounded border border-amber-400/20 bg-amber-400/10 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-amber-300">
                          est
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="h-0.5 rounded-full bg-white/[0.04] w-16 overflow-hidden" aria-hidden>
                        <div className="h-full rounded-full bg-emerald-400/60 transition-all duration-500"
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="h-0.5 rounded-full bg-white/[0.04] w-16 overflow-hidden" aria-hidden>
                        <div className="h-full rounded-full bg-sky-400/60 transition-all duration-500"
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <button
                      onClick={() => { setKillError(null); setToKill(proc) }}
      ...
      <ConfirmationModal
        isOpen={!!toKill}
        onClose={() => { setToKill(null); setKillError(null) }}
        onConfirm={handleKill}
        title="Kill Process?"
        message={
          killError
            ? `Error: ${killError}`
            : `Terminate "${toKill?.name}" (PID: ${toKill?.pid})? This cannot be undone.`
        }
        confirmLabel={isKilling === toKill?.pid ? 'Killing…' : 'Kill Process'}
        isDanger
      />

      {/* Error banner (shown outside modal if modal was already closed) */}
      {killError && !toKill && (
        <div className="mt-3 flex items-center gap-2 rounded-xl border border-rose-500/30 bg-rose-500/[0.06] px-4 py-3 text-[12px] text-rose-300">
