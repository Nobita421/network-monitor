...
import { Activity, AlertTriangle, ArrowDown, ArrowUp, Lock, Unlock, WifiOff } from 'lucide-react'
import type { AlertPayload, LatencyPayload } from '../../lib/ipc'
...
const STALENESS_THRESHOLD_MS = 10_000 // 10s without update = stale

function getPingColorClass(ping?: number) {
  if (typeof ping !== 'number' || ping <= 0) return 'text-slate-400'
  if (ping < 50)  return 'text-emerald-400'
  if (ping < 100) return 'text-amber-400'
  return 'text-rose-400'
}

function mergeOverlayStats(prev: NetworkStat, next: NetworkStat): NetworkStat {
  const merged = { ...prev, ...next, ping: next.ping ?? prev.ping }

  if (
    prev.rx_sec === merged.rx_sec &&
    prev.tx_sec === merged.tx_sec &&
    prev.ping === merged.ping &&
    prev.iface === merged.iface &&
    prev.operstate === merged.operstate
  ) {
    return prev
  }
...
export function OverlayView() {
  const [stats, setStats] = useState<NetworkStat>({
    rx_sec: 0, tx_sec: 0, iface: '', operstate: 'up', ping: 0,
  })
  ...
  useEffect(() => {
    const loadInitialStats = async () => {
      try {
        ...
        if (data) {
          setStats((prev) => mergeOverlayStats(prev, data))
          resetStalenessTimer()
        }
      } catch (error) {
    ...
    const unsubscribeLatency = window.desktop.onLatencyUpdate((payload: LatencyPayload) => {
      setStats((prev) => (
        prev.ping === payload.ping ? prev : { ...prev, ping: payload.ping }
      ))
    })

    const unsubscribeAlert = window.desktop.onAlertTriggered((payload: AlertPayload) => {
      setAlert({ title: payload.title, body: payload.body, time: payload.time })
  ...
  const toggleLock = useCallback(async () => {
    ...
    try {
      ...
      if (success) setIsLocked(!isLocked)
    } catch (error) {
    ...
    <>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div
          className={[
            'pointer-events-auto',
            'flex items-center gap-3 px-3 py-1.5 rounded-full backdrop-blur-md border shadow-lg transition-[background-color,border-color,box-shadow,opacity,transform] duration-300 select-none',
            alert
              ? 'bg-rose-950/90 border-rose-500/50 shadow-rose-500/30 shadow-2xl scale-105'
              : isStale
                ? 'bg-slate-950/70 border-amber-500/30 opacity-70'
                : isLocked
        ...
        >
          ...
          {isStale && !alert && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-amber-400 font-semibold">stale</span>
            </div>
          )}
          ...
          <button
            ...
            className={[
              ...
              isLocked ? 'text-rose-400 hover:bg-rose-500/20' : 'text-slate-400 hover:bg-white/10 hover:text-white',
            ].join(' ')}
