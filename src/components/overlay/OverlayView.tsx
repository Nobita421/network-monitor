...
const StatItem = memo(({ label, value, unit, colorClass, Icon, shadowColor }: any) => {
    const shadowStyle = `0 0 10px ${shadowColor}`;
    return (
    <div className="flex flex-col items-center gap-1 min-w-[70px]">
        ...
        <span className="text-xl font-bold font-mono text-white tracking-tight [text-shadow:0_0_10px_rgba(52,211,153,0.3)]" data-shadow-color={shadowColor}>
            {value}
        </span>
...
export function OverlayView() {
    ...
    return (
        <div className="h-screen w-screen flex items-center justify-center bg-transparent overflow-hidden">
            {/* Draggable Container */}
            <div
                className="w-full h-full flex items-center justify-between px-6 bg-slate-950/90 backdrop-blur-md border border-white/5 shadow-2xl select-none cursor-move"
            >
                <StatItem 
                    label="Down" 
                    value={rxParts[0]} 
