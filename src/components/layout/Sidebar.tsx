...
export function Sidebar({ activeTab, setActiveTab, stats, elapsedSeconds }: SidebarProps) {
  return (
    <aside className="hidden flex-col border-r border-white/5 bg-slate-950/70 backdrop-blur md:flex md:w-64 xl:w-72">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-sm font-medium transition-all ${active
      ...
      <div className="space-y-1 border-t border-white/5 px-6 py-6 text-xs text-slate-400">
